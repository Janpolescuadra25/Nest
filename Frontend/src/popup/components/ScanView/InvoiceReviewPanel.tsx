...
export default function InvoiceReviewPanel({ header, lineItems, confidence, onConfirm, onRetry, onClear }: InvoiceReviewPanelProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
      <div>
        <div className="text-sm font-semibold text-gray-700">Review OCR results</div>
        {confidence !== null && (
          <div className={`text-xs ${confidence < 50 ? 'text-red-600' : confidence < 75 ? 'text-amber-600' : 'text-emerald-600'}`}>
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={field.key} className="grid grid-cols-[120px_minmax(0,1fr)] gap-3 items-center">
            <label className="text-gray-600 text-xs" htmlFor={field.key}>{field.label}</label>
            <input
              id={field.key}
              type="text"
      ...
      <div className="space-y-2">
        <div className="text-xs text-gray-600 font-medium">Line items</div>
        <div className="grid grid-cols-[1.5fr_56px_96px_96px] gap-2 text-xs text-gray-600 font-medium">
          <div>Description</div>
          <div>Qty</div>
          <div>Unit Price</div>
          <div>Total</div>
        ...
        {editLineItems.map((item, index) => (
          <div key={index} className="grid grid-cols-[1.5fr_56px_96px_96px] gap-2 text-gray-900">
            <input
              type="text"
              value={item.description ?? ''}
              onChange={(e) => updateLineItem(index, 'description', e.target.value)}
              className="flex-1 min-w-0 bg-[#F5F5F7] border border-gray-300 text-gray-900 text-xs rounded px-2 py-1 focus:border-emerald-500 focus:outline-none"
            />
            <input
              type="number"
              min="0"
      ...
      <div className="flex gap-2 mt-4">
        ...
        <button
          type="button"
          onClick={onRetry}
          className="border border-gray-300 text-gray-600 rounded px-3 py-2 text-sm hover:bg-gray-200"
        >
          Re-scan
        </button>
        <button
          type="button"
          onClick={onClear}
          className="text-gray-600 text-sm hover:text-gray-600 px-2"
        >
          Clear
        </button>
      </div>
