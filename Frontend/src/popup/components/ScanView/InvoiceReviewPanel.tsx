...
export default function InvoiceReviewPanel({ header, lineItems, confidence, onConfirm, onRetry, onClear }: InvoiceReviewPanelProps) {
  ...
  return (
    <div className="bg-white border rounded-lg p-3 space-y-3">
      <div>
        <div className="text-sm font-semibold text-gray-900">Review OCR results</div>
        {confidence !== null && (
          <div className={`text-xs ${confidence < 50 ? 'text-red-600' : confidence < 75 ? 'text-yellow-600' : 'text-green-600'}`}>
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={field.key} className="grid grid-cols-[120px_minmax(0,1fr)] gap-3 items-center">
            <label className="text-gray-500 text-xs" htmlFor={field.key}>{field.label}</label>
            <input
              id={field.key}
              type="text"
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-[1.5fr_56px_96px_96px] gap-2 text-xs text-gray-500 font-medium">
          <div>Description</div>
          <div>Qty</div>
          <div>Unit Price</div>
          <div>Total</div>
        ...
        {editLineItems.map((item, index) => (
          <div key={index} className="grid grid-cols-[1.5fr_56px_96px_96px] gap-2">
            <input
              type="text"
