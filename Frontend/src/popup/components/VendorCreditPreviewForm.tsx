...
export default function VendorCreditPreviewForm({
  ...
  }, [vendorRef, txnDate, apAccountRef, memo, privateNote, effectiveLines, docNumber]);

  const duplicateCheck = useDuplicateCheck(jwt, 'VENDOR_CREDIT', dedupPayload);
  ...
  const preSyncChecks: PreSyncCheck[] = [
    ...
    },
    {
      ...
      label: 'Total matches scanned credit memo',
      detail:
        totalMismatch !== null
  ...
  return (
    <div className="p-3 flex flex-col gap-3">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-3 grid grid-cols-2 gap-4">
        <div>
          <div className="text-sm font-medium text-gray-700 mb-1">Credit Date</div>
        ...
        <div className="col-span-2">
          <div className="text-sm font-medium text-gray-700 mb-1">Credit No.</div>
          <input
            className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm bg-white text-gray-900 focus:border-emerald-500 focus:outline-none"
            value={docNumber}
      ...
      <div className="flex gap-2">
        ...
        >
          {syncing
            ? (canSyncDirectly ? 'Syncing Vendor Credit…' : 'Submitting…')
            : !hasHeader
              ...
              : !hasAmount
                ...
                : !allMapped
                  ...
                  : (canSyncDirectly ? '⚡ Sync Vendor Credit to QuickBooks' : '📋 Submit for Approval')}
        </button>
      </div>
    </div>
  );
