...
const SECTIONS: Section[] = [
  {
    ...
    content: (
      <div className="space-y-3">
        <p className="text-gray-600 text-xs font-medium">Welcome to Solyra! Here's how to go from zero to your first sync:</p>
        <div className="space-y-2.5">
          <Step num={1} title="Log In">
            Open the Solyra extension and sign in with your email and password. If you don't have an account yet, click <span className="text-emerald-400">"Request Access"</span> and an admin will approve you.
          </Step>
          <Step num={2} title="Connect QuickBooks">
            Go to the <span className="text-emerald-400">Settings</span> tab and click <span className="text-emerald-400">"Connect to QuickBooks"</span>. This opens an Intuit login page — authorize Solyra to access your QuickBooks company. You'll see a green "✅ QB Connected" banner when it's done.
          </Step>
          ...
          <Step num={4} title="Scan a POS Report">
            Navigate to a POS report page (Toast, SALIDO, or Oracle) in your browser, or upload a vendor invoice/check image or Excel file. Then click the <span className="text-emerald-400">Scan</span> tab and scan. Solyra extracts the data automatically.
          </Step>
    ...
    content: (
      <div className="space-y-2">
        ...
        <TabInfo icon="🗺️" name="Mapping">
          Links each scan field to a QuickBooks account. This is where you tell Solyra which account each field belongs to.
        </TabInfo>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
<p className="text-gray-600 text-xs font-medium">Mappings tell Solyra which QuickBooks account each POS field belongs to.</p>
        ...
        <ul className="space-y-1.5">
          <Bullet>Click <span className="text-emerald-400">"Auto-Detect"</span> to let Solyra suggest mappings based on field names and your QB account list</Bullet>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        ...
        <p className="text-[11px] text-gray-600">
          Solyra shows a red <span className="text-red-600">imbalance warning</span> at the bottom of the Preview tab. Common causes: unmapped fields, rounding differences, or missing offset entries. Solyra can auto-fix rounding differences up to $0.02.
        </p>
      </div>
    ),
  },
    ...
    content: (
      <div className="space-y-3">
        ...
        <ul className="space-y-1.5">
          ...
          <Bullet><span className="text-emerald-400">Memo:</span> Private note attached to the entry. You can set a template in Settings per location (e.g., "Solyra sync — {'{date}'} — Downtown").</Bullet>
        </ul>
    ...
    content: (
      <div className="space-y-3">
        <div className="space-y-2">
          ...
          <TroubleshootItem problem="Unbalanced entry" solution="Check that all fields are mapped. Unmapped fields won't have accounts assigned. Look for the red imbalance warning at the bottom of Preview. Solyra auto-fixes rounding up to $0.02." />
          ...
          <TroubleshootItem problem="Token expired" solution='Go to Settings and reconnect QuickBooks. Tokens expire periodically — Solyra auto-refreshes them, but if the refresh token itself expires, you need to re-authorize.' />
          <TroubleshootItem problem="Negative amounts showing" solution="Solyra automatically flips the posting side for negative amounts (a negative Credit becomes a Debit, and vice versa). This is correct behavior — check that your mapping's Debit/Credit setting is right for the positive case." />
        </div>
      </div>
    ),
  },
    ...
    content: (
      <div className="space-y-2">
        <div className="bg-gray-100 rounded-lg p-2 space-y-1.5">
          <Shortcut keys="Ctrl+Shift+N" description="Open Solyra floating window (click extension icon)" />
  ...
  },
  {
    ...
    content: (
      <div className="space-y-2.5">
        <FAQItem q="Does Solyra modify my POS data?" a="No. Solyra only reads data from the POS page. It never writes, modifies, or deletes anything in your POS." />
        <FAQItem q="Can I undo a sync?" a="Solyra doesn't have an undo button, but you can delete the journal entry directly in QuickBooks Online. Find it by the DocNumber or date." />
        ...
        <FAQItem q="Is my data secure?" a="Solyra uses JWT authentication, encrypted OAuth tokens, and never stores your QuickBooks credentials. All API calls use HTTPS. Tokens are stored in your PostgreSQL database." />
      </div>
    ),
  },
  {
      ...
      <div className="space-y-3">
        ...
        <div className="bg-gray-100 rounded-lg p-3 space-y-2">
          <div className="flex items-center gap-2">
            ...
            >
              support@solyra.cloud
            </a>
          </div>
          <p className="text-[11px] text-gray-600">Include your browser version, Solyra version (1.0.0), and steps to reproduce the issue.</p>
        </div>
