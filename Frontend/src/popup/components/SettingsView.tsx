...
export default function SettingsView({ jwt, user, onLogout }: Props) {
  ...
  return (
    <div className="p-3 space-y-4">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3 space-y-3">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {user.subscriptionSource === 'owner' ? null : user.subscriptionSource === 'stripe' ? (
              ...
              <button
                type="button"
                onClick={() => { window.location.href = 'mailto:support@autobooks.cloud?subject=AutoBooks Billing Inquiry'; }}
            ...
            <button
              type="button"
              onClick={() => { window.location.href = 'mailto:support@autobooks.cloud?subject=AutoBooks Billing Help'; }}
              className="w-full rounded-lg border border-gray-200 bg-[#F5F5F7] text-gray-600 py-2 text-xs font-semibold hover:bg-gray-50"
            >
              Billing help
            </button>
      ...
      {/* About */}
      <div>
        <div className="text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">About {user.brandName || 'AutoBooks'}</div>
        ...
        >
          {user.logoUrl ? (
            <img src={user.logoUrl} alt={user.brandName || 'AutoBooks'} className="h-10 w-10 rounded object-contain" />
          ) : (
            <span className="text-3xl leading-none">🪹</span>
          )}
          <div>
            <div className="text-gray-900 font-bold text-sm">{user.brandName || 'AutoBooks'}</div>
        ...
        <div className="from-white to-gray-50 border border-gray-200 rounded-lg p-3 mb-3">
          ...
          <div className="text-gray-600 text-xs mt-1 leading-relaxed">
            ...
            AutoBooks was built to solve the real-world challenge of bridging POS sales data with QuickBooks accounting,
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3 mb-3">
          ...
          >
            <span>✉️</span>
            <span>support@autobooks.cloud</span>
          </a>
          <div className="flex gap-2">
            <a
              href="mailto:support@autobooks.cloud?subject=AutoBooks%20Bug%20Report"
            ...
            <a
              href="mailto:support@autobooks.cloud?subject=AutoBooks%20Feature%20Request"
              className="flex-1 text-center text-xs bg-gray-200 hover:bg-gray-100 text-gray-600 hover:text-gray-900 py-1.5 rounded transition-colors"
            >
              💡 Feature Request
            </a>
