...
export default function ScanView({
  ...
  useEffect(() => {
    ...
    const loadScanPacks = async () => {
      try {
        ...
        if (!active) return;
        console.error('[AutoBooks Popup] Failed to load scan packs:', err);
      }
    };
    ...
    return new Promise((resolve) => {
      const timeout = setTimeout(() => {
        if (process.env.NODE_ENV !== 'production') {
          console.warn('[AutoBooks Popup] Scan timed out for tab', tabId);
        }
        resolve(null);
      }, 30000);
      ...
      chrome.tabs.sendMessage(tabId, { type: 'REQUEST_SCAN' }, (resp) => {
        ...
        if (chrome.runtime.lastError) {
          if (process.env.NODE_ENV !== 'production') {
            console.warn('[AutoBooks Popup] sendMessage error:', chrome.runtime.lastError.message);
          }
          resolve(null);
        } else {
          resolve(resp);
  ...
  const loadAllTabs = async () => {
    try {
      ...
      console.error('[AutoBooks Popup] Failed to load tabs:', err);
    ...
    try {
      ...
      if (!response) {
        ...
        try {
          ...
          console.error('[AutoBooks Popup] Failed to inject content script:', injectErr);
      ...
      if (response?.data) {
        ...
        if (locationId) {
          try {
            ...
            console.error('[AutoBooks] Failed to save scan to backend:', saveErr);
          }
        }
      } else {
      ...
      console.error('[AutoBooks Popup] Scan error:', err);
    ...
    try {
      ...
      if (locationId) {
        try {
          ...
          console.error('[AutoBooks] Failed to save AI scan to backend:', saveErr);
        }
      }
    } catch (err: any) {
      if (err instanceof ApiError && err.status === 403 && err.payload?.error === 'SCAN_LIMIT_REACHED') {
      ...
      console.error('[AutoBooks Popup] AI scan error:', err);
      ...
      if (locationId) {
        ...
        for (const entry of parsedEntries) {
          try {
            ...
            console.error(`[AutoBooks] Failed to save Excel entry ${entry.id} (row ${entry.rowNumber ?? '?'}) :`, saveErr);
            failCount++;
          }
        }
        if (failCount > 0) {
            ...
            <div className="space-y-2">
              {selectedTab ? (
                ...
                <div className="space-y-1 text-xs text-gray-600">
                  <div>Pick a tab above to scan it with AutoBooks.</div>
