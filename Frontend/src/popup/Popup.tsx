...
class ErrorBoundary extends React.Component<{ children?: React.ReactNode }, { hasError: boolean; error: Error | null }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="bg-[#F5F5F7] text-gray-900 min-h-screen flex flex-col items-center justify-center p-4">
          <div className="text-4xl mb-4">⚠</div>
          <h1 className="text-xl font-semibold mb-2">Something went wrong</h1>
          <p className="text-sm text-gray-600 mb-4 text-center max-w-sm">
            An unexpected error occurred while loading AutoBooks. Refresh the popup to try again.
          </p>
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={() => { window.location.href = 'mailto:support@autobooks.cloud'; }}
