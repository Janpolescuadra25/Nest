...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;background:#f8fafc;margin:0;padding:40px 20px;">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,0.08);">
    <div style="background:#0f172a;padding:24px 32px;">
      <span style="color:#22d3ee;font-size:22px;font-weight:700;letter-spacing:-0.5px;">Solyra</span>
    </div>
    <div style="padding:32px;">
      ${body}
    </div>
    <div style="background:#f8fafc;padding:16px 32px;border-top:1px solid #e2e8f0;">
      <p style="color:#94a3b8;font-size:12px;margin:0;">This message was sent automatically by Solyra. If you didn't expect this email, you can safely ignore it.</p>
    </div>
  </div>
</body>
</html>`;
  ...
  try {
    ...
    const html = emailWrapper(`
      ...
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;">
        Your Solyra account has been created. You can log in with the email address this was sent to.
      </p>
      ...
      <ol style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;padding-left:20px;">
        <li>Install the Solyra extension from the Chrome Web Store</li>
        <li>Log in with your email and the password above</li>
        <li>Connect your QuickBooks Online account</li>
        <li>Invite your team members</li>
      </ol>
      <p style="margin:0 0 24px;"><a href="https://chromewebstore.google.com/detail/nest-restaurant-financial/ccghhfmkjbcakhnoamgihifonfiammoc" style="color:#22d3ee;text-decoration:none;">Install Solyra from the Chrome Web Store</a></p>
    `);

    await resend.emails.send({
      from: process.env.RESEND_FROM_ADDRESS ?? 'noreply@solyra.cloud',
      to,
      subject: 'Welcome to Solyra',
      html,
    });
...
export async function sendVerificationEmail({
  ...
  try {
    ...
    const html = emailWrapper(`
      <p style="color:#1e293b;font-size:16px;margin:0 0 16px;">Hi ${escapeHtml(displayName)},</p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;">
        Thanks for signing up for Solyra. Please verify your email address by clicking the button below.
      </p>
      ...
      <p style="color:#64748b;font-size:13px;line-height:1.6;margin:0;">
        If you did not sign up for Solyra, you can safely ignore this email.
      </p>
    `);
    ...
    await resend.emails.send({
      from: process.env.RESEND_FROM_ADDRESS ?? 'noreply@solyra.cloud',
      to,
      subject: 'Verify your Solyra email',
      html,
    });
    if (process.env.NODE_ENV !== 'production') {
...
export async function sendTrialWarning({
  ...
  try {
    ...
    const html = emailWrapper(`
      <p style="color:#1e293b;font-size:16px;margin:0 0 16px;">Hi ${escapeHtml(displayName)},</p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 16px;">
        Your Solyra trial access will expire on <strong style="color:#0f172a;">${formattedDate}</strong>.
    ...
    await resend.emails.send({
      from: process.env.RESEND_FROM_ADDRESS ?? 'noreply@solyra.cloud',
      to,
      subject: `Your Solyra trial expires in ${daysRemaining} ${dayLabel}`,
      html,
    });
    if (process.env.NODE_ENV !== 'production') {
...
export async function sendSyncFailureAlert({
  ...
  try {
    ...
    const html = emailWrapper(`
      <p style="color:#1e293b;font-size:16px;margin:0 0 16px;">Hi ${escapeHtml(displayName)},</p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 16px;">
        There are <strong style="color:#0f172a;">${total}</strong> scan${total !== 1 ? 's' : ''} that need attention in Solyra.
...
export async function sendTrialExpired({
  ...
  try {
    ...
    const html = emailWrapper(`
      <p style="color:#1e293b;font-size:16px;margin:0 0 16px;">Hi ${escapeHtml(displayName)},</p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 16px;">
        Your Solyra trial access expired on <strong style="color:#0f172a;">${formattedDate}</strong>.
    ...
    await resend.emails.send({
      from: process.env.RESEND_FROM_ADDRESS ?? 'noreply@solyra.cloud',
      to,
      subject: 'Your Solyra trial has expired',
      html,
    });
    if (process.env.NODE_ENV !== 'production') {
...
export async function sendTrialRenewed({
  ...
  try {
    ...
    const html = emailWrapper(`
      <p style="color:#1e293b;font-size:16px;margin:0 0 16px;">Hi ${escapeHtml(displayName)},</p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 16px;">
        Your Solyra trial has been renewed by your administrator.
    ...
    await resend.emails.send({
      from: process.env.RESEND_FROM_ADDRESS ?? 'noreply@solyra.cloud',
      to,
      subject: 'Your Solyra trial has been renewed',
      html,
    });
    if (process.env.NODE_ENV !== 'production') {
...
export async function sendPasswordResetEmail({
  ...
  try {
    ...
    const html = emailWrapper(`
      <p style="color:#1e293b;font-size:16px;margin:0 0 16px;">Hi ${escapeHtml(displayName)},</p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;">
        We received a request to reset your Solyra password. Click the link below to set a new password.
      </p>
      <div style="text-align:center;margin:0 0 24px;">
        <a href="${resetLink}"
           style="display:inline-block;background:#22d3ee;color:#0f172a;font-size:16px;font-weight:700;text-decoration:none;padding:14px 32px;border-radius:8px;letter-spacing:0.02em;">
    ...
    await resend.emails.send({
      from: process.env.RESEND_FROM_ADDRESS ?? 'noreply@solyra.cloud',
      to,
      subject: 'Reset your Solyra password',
      html,
    });
    if (process.env.NODE_ENV !== 'production') {
...
export async function sendRejectionEmail({
  ...
  try {
    ...
    const html = emailWrapper(`
      <p style="color:#1e293b;font-size:16px;margin:0 0 16px;">Hi ${escapeHtml(displayName)},</p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;">
        Thank you for your interest in becoming an Solyra partner.
      </p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;">
      ...
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;">
        If you have questions, reach out to us at <a href="mailto:support@solyra.cloud" style="color:#22d3ee;text-decoration:none;">support@solyra.cloud</a>.
      </p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;">
        Reapply anytime by visiting the Solyra website: <a href="https://solyra.cloud" style="color:#22d3ee;text-decoration:none;">https://solyra.cloud</a>
      </p>
      <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 16px;">Best regards,<br/>The Solyra Team</p>
    `);

    await resend.emails.send({
      from: process.env.RESEND_FROM_ADDRESS ?? 'noreply@solyra.cloud',
      to,
      subject: 'Update on Your Solyra Partner Application',
      html,
    });
    if (process.env.NODE_ENV !== 'production') {
